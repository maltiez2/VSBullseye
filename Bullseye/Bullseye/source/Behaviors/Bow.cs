...
internal class BowBehavior : RangedWeaponBehavior
{
    ...
    public BowBehavior(CollectibleObject collObj) : base(collObj) { }
    ...
    public override void OnLoaded(ICoreAPI api)
    {
        ...
        if (Stats != null) Stats.WeaponType = WeaponsType.Bow;
    ...
    public override void OnAimingStart(ItemSlot slot, EntityAgent byEntity)
    {
        ...
        StartDrawAnimation(GetChargeNeeded(_api, byEntity), byEntity);
    ...
    public override void OnAimingStep(float secondsUsed, ItemSlot slot, EntityAgent byEntity)
    {
        //if (byEntity.World is IClientWorldAccessor)
        {
            ...
            int renderVariant = GameMath.Clamp((int)Math.Ceiling(secondsUsed * 3f / GetChargeNeeded(_api, byEntity)), 0, 4);
        ...
        if (byEntity.World is IClientWorldAccessor)
        {
            SetAimAnimation(_api as ICoreClientAPI);
        }
    }
    ...
    public override void OnAimingCancel(float secondsUsed, ItemSlot slot, EntityAgent byEntity, EnumItemUseCancelReason cancelReason)
    {
        ...
        if (byEntity.World is IClientWorldAccessor)
        {
            ...
            ResetAimAnimation(_api as ICoreClientAPI, byEntity);
        }

        slot.Itemstack?.Attributes.SetInt("renderVariant", 0);
        (byEntity as EntityPlayer)?.Player?.InventoryManager.BroadcastHotbarSlot();

        if (cancelReason != EnumItemUseCancelReason.ReleasedMouse || secondsUsed < GetChargeNeeded(_api, byEntity))
        {
    ...
    public override float GetProjectileDamage(EntityAgent byEntity, ItemSlot weaponSlot, ItemSlot ammoSlot)
    {
        ...
        damage *= Config?.GetDamage(Stats?.WeaponType ?? WeaponsType.Bow) ?? 1f;
    ...
    public override void OnShot(ItemSlot slot, Entity projectileEntity, EntityAgent byEntity)
    {
        ...
        _api.Event.RegisterCallback((ms) =>
        {
            byEntity.AnimManager.StopAnimation("bowaim");
        }, 500);
    }
    ...
    public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
    {
        ...
        float dmg = inSlot.Itemstack.ItemAttributes["damage"].AsFloat(0) * Config?.GetDamage(Stats?.WeaponType ?? WeaponsType.Bow) ?? 1f;
    ...
    protected void SetAimAnimation(ICoreClientAPI capi)
    {
        ...
        Vec2f? currentAim = Aiming?.GetCurrentAim();
    ...
    private void PrepareAnimations()
    {
        ...
        _animationManager = _api.ModLoader.GetModSystem<AnimationManagerLibSystem>();
