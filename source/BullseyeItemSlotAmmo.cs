...
namespace Bullseye
{
	public class BullseyeItemSlotAmmo : ItemSlot
	{
		...
		public override string GetStackDescription(IClientWorldAccessor world, bool extendedDebugInfo)
		{
			...
			{
				stackDescription += "\n" + Lang.Get("bullseye-continued:weapon-ranged-total-damage", weaponDamage, breakChance * 100f, weaponName);
			}
			else
			{
				stackDescription += "\n" + Lang.Get("bullseye-continued:weapon-ranged-total-damage-no-drops", weaponDamage, weaponName);

				if (breakChance >= 1f) stackDescription += "\n" + Lang.Get("bullseye-continued:projectile-always-breaks");
			}
